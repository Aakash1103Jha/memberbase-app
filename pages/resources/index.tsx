	...
	return (
		<div className={`${styles.resource_card}`}>
			<div className={`${styles.resource_cover}`}>
				<Image
					src="https://mir-s3-cdn-cf.behance.net/project_modules/disp/e9617d19293361.562d7f22ac57d.gif"
					width={200}
					height={200}
					alt={data.title}
				/>
				<div className={`${styles.resource_publish_info}`}>
					<p>{data.pages} pages</p>
					{/* <p>{Intl.DateTimeFormat("en-IN", { dateStyle: "medium" }).format(new Date(data.published))}</p> */}
				</div>
			</div>
			<div className={`${styles.resource_card_info}`}>
				<p className={`${styles.resource_author}`}>{data.author}</p>
				<p className={`${styles.resource_subtitle}`}>{data.subtitle}</p>
				<h2 className={`${styles.resource_title}`}>{data.title}</h2>
			</div>
		</div>
	);
};
			...
			<div className={`${styles.resources_page}`}>
				<PageContainer>
					...
					<GenericList
						...
						keyExtractor={({ id }) => id}
						element={(book) => (
							<Link key={book.isbn} href={`/resources/${book.isbn}`} passHref>
								<ResourceCard {...book} />
							</Link>
						)}
					/>
